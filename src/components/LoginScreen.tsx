        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 shadow-sm">
             ...
             <span className="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest">
               {isConnecting ? 'Подключение...' : isCloudActive ? 'В сети (Облако)' : 'Автономный режим'}
             </span>
          ...
          {connectionError && !isCloudActive && !isConnecting && (
            <p className="mt-2 text-[10px] text-slate-400 dark:text-slate-500 max-w-[200px] mx-auto italic">
              {connectionError.includes('auth/network-request-failed') ? 'Ошибка сети' : 'Сервис временно недоступен'}
            </p>
          )}
        </motion.div>
      </motion.div>
